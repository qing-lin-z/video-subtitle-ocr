...
class ROISelector:
    ...
    @staticmethod
    def auto_detect(video_path: str,
                    sample_frames: int = 6,
                    box_thresh: float = 0.2,
                    min_width_ratio: float = 0.15,
                    margin: int = 8) -> tuple[int, int, int, int]:
        """
        自动检测字幕区域：采样关键帧 → 文字检测 → 聚合 → 返回 ROI

        Args:
            video_path: 视频文件路径
            sample_frames: 采样帧数 (默认 6)
            box_thresh: 检测阈值 0~1，越低越敏感 (默认 0.2)
            min_width_ratio: 最小宽度占画面宽度的比例 (默认 0.15)
            margin: ROI 边距像素 (默认 8)
        ...
        print(f"  📐 {fw}×{fh}  |  {fps:.2f} FPS  |  {timedelta(seconds=int(duration))}")

        # ── 选取采样帧位置：跳过开头 15%，从中间均匀采样 ──
        start_ratio = 0.15
        end_ratio = 0.85
        ...
        # ── 初始化 OCR (只做文字检测) ──
        print(f"  ⏳ 正在加载文字检测引擎...")
        detector = RapidOCR(box_thresh=box_thresh,
                            text_score=0.1,
                            use_text_det=True,
                            use_angle_cls=False)
        ...
        for idx, fn in enumerate(frame_indices):
            ...
            # 只做文字检测 (不识别)，速度快
            dt_boxes, dt_elapse = detector.text_detector(frame)
            perc = int((idx + 1) / len(frame_indices) * 100)
            print(f"  [{perc:3d}%] 帧#{fn:6d} 检出 {len(dt_boxes):2d} 个文字区域 "
                  f"({dt_elapse*1000:.0f}ms)")
        ...
        if not all_boxes:
            print("\n  ⚠️ 未检测到任何文字区域，请手动框选")
            return ROISelector.select(video_path)

        min_w = fw * min_width_ratio
        max_h = fh * 0.15
        bottom_start = fh * 0.55  # 画面下 45% 区域
        ...
        print(f"\n  ✅ 自动检测完成: x={x} y={y} w={w} h={h}")
        print(f"  📊 共 {len(candidate_boxes)} 个候选框, "
              f"采样 {len(all_boxes)} 帧\n")
...
示例:
  ...
  %(prog)s video.mp4 --auto-detect              # 自动检测字幕区域
  %(prog)s video.mp4 --auto-detect --save-roi   # 自动检测+保存
...
RapidOCR 引擎 — 基于 ONNX Runtime，内置中英文识别，无需配置语言
    ...
    roi = None
    if args.auto_detect:
        roi = ROISelector.auto_detect(args.video)
        if args.save_roi:
            ROISelector.save(roi, roi_file)
    elif args.no_select:
        roi = ROISelector.load(roi_file)
        if roi is None:
            print("  ⚠️ 未找到ROI配置文件，进入自动检测...")
            roi = ROISelector.auto_detect(args.video)
            if args.save_roi:
                ROISelector.save(roi, roi_file)
    else:
